...
#ifndef ae_mat4_neon_h
#define ae_mat4_neon_h
...
AE_INLINE
void
ae_mat4_scale_neon(mat4 m, float s) {
  float32x4_t v0;
...
AE_INLINE
void
ae_mat4_transp_neon(mat4 m, mat4 dest) {
  float32x4x4_t vmat;
...
AE_INLINE
void
ae_mat4_mul_neon(mat4 m1, mat4 m2, mat4 dest) {
  /* D = R * L (Column-Major) */
  ...
  v0 = vmulq_f32(aem_splat_x(r0), l);
  v1 = vmulq_f32(aem_splat_x(r1), l);
  v2 = vmulq_f32(aem_splat_x(r2), l);
  v3 = vmulq_f32(aem_splat_x(r3), l);
...
AE_INLINE
void
ae_mat4_mulv_neon(mat4 m, vec4 v, vec4 dest) {
...
ae_mat4_det_neon(mat4 mat) {
  ...
  r1 = vrev64q_f32(aem_load(mat[1])); /* g h e f */
  r2 = vrev64q_f32(aem_load(mat[2])); /* l k i j */
  r3 = vrev64q_f32(aem_load(mat[3])); /* o p m n */
  ...
  /*
   ...
   t[4] = i * o - m * k;
   */
  x0 = aem_fnmadd(vcombine_f32(kl, kl), vcombine_f32(nn, mm),
  ...
  /*
   ...
   t[5] = i * n - m * j;
   */
  x1 = aem_fnmadd(vcombine_f32(vdup_lane_f32(kl, 0), jj),
  ...
  /*
   ...
   - d * (e * t[2] - f * t[4] + g * t[5])
   */
  x2 = aem_fnmadd(vcombine_f32(vdup_lane_f32(gh, 1), vdup_lane_f32(ef, 0)),
                   vcombine_f32(vget_low_f32(a1.val[0]), t34),
                   vmulq_f32(vcombine_f32(ef, vdup_lane_f32(ef, 1)),
                             vcombine_f32(vget_low_f32(x1), t12)));

  x2 = aem_fmadd(vcombine_f32(vdup_lane_f32(gh, 0), gh),
                  vcombine_f32(vget_low_f32(a1.val[1]), vget_high_f32(x1)), x2);

  x2 = aem_xor(x2, x3);

  return aem_hadd(vmulq_f32(x2, r0));
}

AE_INLINE
void
ae_mat4_inv_neon(mat4 mat, mat4 dest) {
  ...
  /* t1[0] = k * p - o * l;
     ...
     t2[0] = g * p - o * h;
     t3[0] = g * l - k * h; */
  t0 = aem_fnmadd(x2, x0, vmulq_f32(x3, x1));
  ...
  /* t1[1] = j * p - n * l;
     ...
     t2[1] = f * p - n * h;
     t3[1] = f * l - j * h; */
   t1 = aem_fnmadd(x4, x0, vmulq_f32(x5, x1));
  
  /* t1[2] = j * o - n * k
     t1[2] = j * o - n * k;
     t2[2] = f * o - n * g;
     t3[2] = f * k - j * g; */
  t2 = aem_fnmadd(x4, x3, vmulq_f32(x5, x2));
  ...
  /* t1[3] = i * p - m * l;
     ...
     t2[3] = e * p - m * h;
     t3[3] = e * l - i * h; */
  t3 = aem_fnmadd(x7, x0, vmulq_f32(x6, x1));
  
  /* t1[4] = i * o - m * k;
     t1[4] = i * o - m * k;
     t2[4] = e * o - m * g;
     t3[4] = e * k - i * g; */
  t4 = aem_fnmadd(x7, x3, vmulq_f32(x6, x2));
  
  /* t1[5] = i * n - m * j;
     t1[5] = i * n - m * j;
     t2[5] = e * n - m * f;
     t3[5] = e * j - i * f; */
  t5 = aem_fnmadd(x7, x5, vmulq_f32(x6, x4));
  ...
  v0 = aem_xor(aem_fmadd(x3, t2, aem_fnmadd(x2, t1, vmulq_f32(x1, t0))), x8);
  ...
  v2 = aem_xor(aem_fmadd(x3, t5, aem_fnmadd(x1, t3, vmulq_f32(x0, t1))), x8);
  ...
  v1 = aem_xor(aem_fmadd(x3, t4, aem_fnmadd(x2, t3, vmulq_f32(x0, t0))), x9);
  ...
  v3 = aem_xor(aem_fmadd(x2, t5, aem_fnmadd(x1, t4, vmulq_f32(x0, t2))), x9);
  ...
  /*
  x0 = aem_div(aem_set1(1.0f), aem_vhadd(vmulq_f32(x0, r0)));

  aem_store(dest[0], vmulq_f32(v0, x0));
  aem_store(dest[1], vmulq_f32(v1, x0));
  aem_store(dest[2], vmulq_f32(v2, x0));
  aem_store(dest[3], vmulq_f32(v3, x0));
  */

  x0 = aem_vhadd(vmulq_f32(x0, r0));

  aem_store(dest[0], aem_div(v0, x0));
  aem_store(dest[1], aem_div(v1, x0));
  aem_store(dest[2], aem_div(v2, x0));
  aem_store(dest[3], aem_div(v3, x0));
}

#endif
#endif /* ae_mat4_neon_h */
