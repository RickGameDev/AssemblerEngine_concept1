...
#    define AE_ALIGN(X) /* no alignment */
#  else
#    define AE_ALIGN(X) __declspec(align(X))
#  endif
#else
#  define AE_ALIGN(X) __attribute((aligned(X)))
#endif

#ifndef AE_ALL_UNALIGNED
#  define AE_ALIGN_IF(X) AE_ALIGN(X)
#else
#  define AE_ALIGN_IF(X) /* no alignment */
#endif

#ifdef __AVX__
#  define AE_ALIGN_MAT AE_ALIGN(32)
#else
#  define AE_ALIGN_MAT AE_ALIGN(16)
#endif

#ifdef __GNUC__
#  define AE_ASSUME_ALIGNED(expr, alignment) \
  __builtin_assume_aligned((expr), (alignment))
#else
#  define AE_ASSUME_ALIGNED(expr, alignment) (expr)
#endif

#define AE_CASTPTR_ASSUME_ALIGNED(expr, type) \
  ((type*)AE_ASSUME_ALIGNED((expr), __alignof__(type)))

typedef float                   vec2[2];
typedef float                   vec3[3];
typedef int                    ivec3[3];
typedef AE_ALIGN_IF(16) float vec4[4];
typedef vec4                    versor;     /* |x, y, z, w| -> w is the last */
typedef vec3                    mat3[3];
typedef AE_ALIGN_IF(16) vec2  mat2[2];
typedef AE_ALIGN_MAT    vec4  mat4[4];

/*
  Important: ae stores quaternion as [x, y, z, w] in memory since v0.4.0 
...
#define AE_PI       GLM_PIf
#define AE_PI_2     GLM_PI_2f
#define AE_PI_4     GLM_PI_4f
