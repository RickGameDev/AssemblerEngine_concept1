...
#ifndef aes_frustums_h
#define aes_frustums_h
...
AE_INLINE
void
aes_frustum_planes(mat4s m, vec4s dest[6]) {
  vec4 rawDest[6];
  ae_frustum_planes(m.raw, rawDest);
  aes_vec4_pack(dest, rawDest, 6);
}
...
AE_INLINE
void
aes_frustum_corners(mat4s invMat, vec4s dest[8]) {
  vec4 rawDest[8];
  ae_frustum_corners(invMat.raw, rawDest);
  aes_vec4_pack(dest, rawDest, 8);
}
...
AE_INLINE
vec4s
aes_frustum_center(vec4s corners[8]) {
  vec4 rawCorners[8];
  vec4s r;

  aes_vec4_unpack(rawCorners, corners, 8);
  ae_frustum_center(rawCorners, r.raw);
  return r;
}
...
AE_INLINE
void
aes_frustum_box(vec4s corners[8], mat4s m, vec3s box[2]) {
  vec4 rawCorners[8];
  vec3 rawBox[2];

  aes_vec4_unpack(rawCorners, corners, 8);
  ae_frustum_box(rawCorners, m.raw, rawBox);
  aes_vec3_pack(box, rawBox, 2);
}
...
AE_INLINE
void
aes_frustum_corners_at(vec4s corners[8],
                        float splitDist,
                        float farDist,
  ...
  aes_vec4_unpack(rawCorners, corners, 8);
  ae_frustum_corners_at(rawCorners, splitDist, farDist, rawPlaneCorners);
  aes_vec4_pack(planeCorners, rawPlaneCorners, 8);
}

#endif /* aes_frustums_h */
