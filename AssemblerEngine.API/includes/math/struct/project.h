...
#ifndef aes_projects_h
#define aes_projects_h
...
AE_INLINE
vec3s
aes_unprojecti(vec3s pos, mat4s invMat, vec4s vp) {
  vec3s r;
  ae_unprojecti(pos.raw, invMat.raw, vp.raw, r.raw);
  return r;
}
...
AE_INLINE
vec3s
aes_unproject(vec3s pos, mat4s m, vec4s vp) {
  vec3s r;
  ae_unproject(pos.raw, m.raw, vp.raw, r.raw);
  return r;
}
...
AE_INLINE
vec3s
aes_project(vec3s pos, mat4s m, vec4s vp) {
  vec3s r;
  ae_project(pos.raw, m.raw, vp.raw, r.raw);
  return r;
}
...
AE_INLINE
mat4s
aes_pickmatrix(vec3s center, vec2s size, vec4s vp) {
  mat4s res;
  ae_pickmatrix(center.raw, size.raw, vp.raw, res.raw);
  return res;
}

#endif /* aes_projects_h */
