...
#ifndef ae_types_struct_h
#define ae_types_struct_h
 ...
 * compatible, ae doesn't use the anonymous structs internally.
 */
#ifndef AE_USE_ANONYMOUS_STRUCT
...
#  if defined(AE_NO_ANONYMOUS_STRUCT)
     /* The user has defined AE_NO_ANONYMOUS_STRUCT. This used to be the
...
#    define AE_USE_ANONYMOUS_STRUCT 0
...
typedef union AE_ALIGN_IF(16) vec4s {
  vec4 raw;
#if AE_USE_ANONYMOUS_STRUCT
  struct {
    float x;
...
typedef union AE_ALIGN_IF(16) versors {
  vec4 raw;
#if AE_USE_ANONYMOUS_STRUCT
  struct {
    float x;
...
typedef union AE_ALIGN_MAT mat4s {
  mat4  raw;
  vec4s col[4];
#if AE_USE_ANONYMOUS_STRUCT
  struct {
