...
struct ae_os_filesystem_api
{
    ...
    uint32_t                    (*filesystem_it_get_full_path)(struct ae_os_filesystem_it* it, char* dest, uint32_t size);
    bool						(*filesystem_it_is_dir)(struct ae_os_filesystem_it* it);
    uint32_t                    (*filesystem_it_get_name)(struct ae_os_filesystem_it* it, char* dest, uint32_t size);
    uint32_t                    (*filesystem_it_get_name_with_ext)(struct ae_os_filesystem_it* it, char* dest, uint32_t size);
    uint32_t                    (*filesystem_it_get_extension)(struct ae_os_filesystem_it* it, char* dest, uint32_t size);
